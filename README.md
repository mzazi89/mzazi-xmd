...
| Path | What it covers |
|---|---|
| `lib/buttons.js` → `sendButtonMessage` | every `mzazireply` reply — the path most of the pack takes |
| `lib/interactive.js` → `sendInteractiveMessage` | every menu (the panel size/nest/egg steps, the plans picker), and every body that sends one directly |
...
| `lib/waPanel.js` | the prompts that ask the reseller to do something, so they carry Cancel / Try again |
...
| `lib/replyImage.js` | The picture attached to every button message |
| `lib/buttons.js` · `lib/interactive.js` | Button sends — now always with the picture |
| `scripts/test-buttons.js` | Button + picture on every reply path, and the switch |

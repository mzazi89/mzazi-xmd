...
async function generateWelcomeImage({ memberNumber, groupName, memberCount, profilePicUrl }) {
  ...
  const botName = config.botName || "MZAZI TECH XMD BOT";
...
async function generateEventImage({ memberNumber, eventType, groupName, details = "", profilePicUrl }) {
  ...
  const THEMES = {
    ...
    // semantic one (gold = promoted, red = removed), so it follows this bot's
    // accent instead of QUARTZ XD's green.
...
const mzazireply = async (text, options = {}) => {
    ...
    // `.buy` (no plan arg) → MZAZI XMD main menu, as defined in the admin
    ...
    if (command) {
      // Scoped to this bot's profile (xmd), so the two bots answer to

...
//
// The reseller now picks the nest and the egg from menus, so this is read on
// EVERY tap rather than once per creation. A 60-second cache keeps a menu tap
// from costing an API round-trip while still showing a nest the admin has just
// added within a minute — the same trade-off lib/settings.js makes for the panel
// credentials themselves.
let nestCache = null;
let nestCacheAt = 0;
const NEST_TTL = 60 * 1000;

function clearNestCache() {
  nestCache = null;
  nestCacheAt = 0;
}

async function getNests(force = false) {
  const now = Date.now();
  if (!force && nestCache && now - nestCacheAt <= NEST_TTL) return nestCache;
...
// ─── Validate a nest/egg pair the reseller tapped ────────────────────────────
// A menu can outlive what it lists: the admin deletes an egg between the menu
// being sent and the tap. Naming the missing thing is the point — "Nest #7 is
// gone, pick again" is actionable, while an unvalidated id would reach the panel
// API and come back as an opaque 404 halfway through creating the client's user.
async function resolveNestEgg(nestId, eggId) {
  const nests = await getNests();
  if (!nests.length) throw new Error('No Pterodactyl nests are configured on the panel yet.');

  const nest = nests.find((n) => String(n.id) === String(nestId));
  if (!nest) throw new Error(`Nest #${nestId} is no longer on the panel. Send .panel to pick again.`);

  const egg = (nest.eggs || []).find((e) => String(e.id) === String(eggId));
  if (!egg) throw new Error(`That egg is no longer in ${nest.name}. Send .panel to pick again.`);
...
//
// nestId/eggId are what the RESELLER tapped (see lib/waPanel.js). They are
// optional only so that an older caller — or an admin action issued from the
// website — still creates something: with neither given, the first available
// egg is used exactly as this flow always did.
async function createWhatsappPanel({ username, phone, size, resellerPhone, nestId = null, eggId = null }) {
  await botDb.ensureTables();
  ...
  // Generated login password + the nest/egg the reseller chose.
  const password = crypto.randomBytes(8).toString('base64url').slice(0, 12);
  const template =
    nestId !== null && eggId !== null && nestId !== undefined && eggId !== undefined
      ? await resolveNestEgg(nestId, eggId)
      : await pickDefaultNestEgg();
  const { nestName, eggName } = template;
  const pkg = {
  ...
  const panel = await createPanel({
    ...
    password,
    pkg,
    // The RESOLVED pair, which for the auto path is the default pick — passing
    // the caller's raw arguments through would send `undefined` to the panel API
    // whenever the reseller had not chosen yet.
    nestId: template.nestId,
  ...
  try {
    await prisma.$executeRawUnsafe(
      ...
      template.nestId || null, template.eggId || null, eggName || null
    );
  } catch (e) {

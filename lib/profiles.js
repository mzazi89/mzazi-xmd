// ─────────────────────────────────────────────────────────────────────────────
// BOT PROFILES — which WhatsApp identity a paired number belongs to.
//
// THIS BOT SERVES A SINGLE PROFILE: MZAZI XMD (id "xmd"). QUARTZ XD runs as a
// separate process in its own repository with its own profile. Keeping the
// profile machinery here means the shared platform code (pairing, telemetry,
// subscriptions, the website's bot chooser) works unchanged, and this bot
// simply owns one entry rather than two.
//
// Because there is one profile, `primary()` is always "xmd" and every lookup
// resolves to it — so nothing needs to ask the user which bot to use.
//
